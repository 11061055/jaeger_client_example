...
9. DAG 图节点的多少, 是和我们有多少个函数调用 trigger() 一致的(不是调用次数, 相同函数虽然可以被调用多次, 或者一个函数中存在多个 trigger, 但是只对应一个 DAG 图节点). 简单点说 就是代码中有多少个函数写了 trigger(), DAG 图中就会有多少个节点, 即所需额外内存大小也是业务可控的.
```
